...
Данные объявления отражаются в разделе `Объявления`. Объявления наполняются независимо от кода данного сайта путём наполнения папки `adverts`, расположенной папке `public` проекта, и файла `adverts.json` в папке `adverts`, который содержит в формате `json` массив имён папок объявлений.
