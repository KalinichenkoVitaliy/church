...
В случае, когда на один день приходится несколько разных события, но имена папкам-событиям нужно добавлять в конце знак подчёркивания `_` и номер события в этом дне по-порядку (1, 2, 3 и т.д.).
