...
- `content` - массив блоков-тегов, из которых формируется тело контента события. Каждый блог-тег содержит два свойства: сам тег (`tag`) и его значение (`value`).

В приведенном выше примере будет сформировано такое тело контента события:

- абзац текста с текстом из `value` (`indent_p` - html <p> с отступом на первой строке);
