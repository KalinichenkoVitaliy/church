...
В случае, когда на один день приходится несколько разных события, то к имени папкам-событиям нужно добавлять в конце знак подчёркивания `_` и номер события в этом дне по-порядку (1, 2, 3 и т.д.).
