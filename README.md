...
Это многостраничный сайт, где загрузка отдельных страниц-разделов выполняется динамически, что ускоряет первый просмотр сайта. На страницах-разделах находятся: множество информационной и графической информации; форма обратной связи с возможностью тут же направить сообщение приходу храма, которое придёт в виде письма черех сторонний сервис доставки почты; яндекс-карта с указанием месторасположение храма; новостной блок о событиях храма.

Что касается новостного блока, то он состоит их отдельных новостей, контент каждой их которых может содержать и текст и фотографии. Контент каждой новости находится в отдельном внешнем файле. При открытии страницы-раздела динамически прочитываются заголовки всех новостей, а при открытии отдельной новости "лениво" погружается её графический контент, что снижает нагрузку на канал передачи данных при открытии новостного блока в целом. А такая организация хранения данных новостей позволяет пополнять новостной блок сайта путём добавления файлов-новостей и фото-приложений к ним без внесения изменений в код самого сайта.

Помимо всего прочего, контент сайта адаптивен к размерам экрана/окна пользователя как текстового контента, так и графического, т.е. вес и размеры шрифта текста и загружаемых на страницы изображений варьируются в следующих размерах экрана/окна: mobile, tablet, laptop и desktop.
...
Если качество изображения не высокое, то можно не готовить 4 разных по размеру файла изображения, а использовать только один файл без дописывания к нему типов размера экранов. Нужно будет только привести это изображение к формату `webp` (например: `my-image.webp`).

### `События приходской жизни`

Данные события отражаются в разделе `Приходская жизнь`. События приходской жизни наполняются независимо от кода данного сайта путём наполнения папки `news`, расположенной папке `public` проекта, и файла `list.json` в папке `news`, который содержит в формате `json` массив имён папок событий.
...
В случае, когда на один день приходится несколько разных событий, тогда к имени папкам-событиям нужно добавлять в конце знак подчёркивания `_` и номер события в этом дне по-порядку (1, 2, 3 и т.д., например: 2022-08-21_1, 2022-08-21_2, 2022-08-21_3 и т.д.).
...
Данные объявления отражаются в разделе `Объявления`. Объявления наполняются независимо от кода данного сайта путём наполнения папки `adverts`, расположенной папке `public` проекта, и файла `list.json` в папке `adverts`, который содержит в формате `json` массив имён папок объявлений.

Для каждого нового объявления в папке `adverts` создаётся новая папка с именем соответствующем дате окончания этого объявления (например: `2022-10-02`) и это имя добавляется к массиву имён в файле `adverts.json`. Затем в новой папке события создаётся файл-описатель данного события с именем алогичным имени этой папки и типом `json` (например: `2022-10-02.json`). Также, в эту же папку (`2022-10-02`) можно копировать файлы графических изображений по аналогии с событиями приходской жизни.

Структура и назначение папок и файлов объявлений полностью аналогична структуре и назначению папок и файлов событий приходской жизни с той лишь разницей, что дата в объявлениях играет роль дня окончания действия объявления (включительно).
